package fr.maxlego08.bourse.listener;
...
public class AdapterListener extends ZUtils implements Listener {
	...
	private final BoursePlugin template;

	public AdapterListener(BoursePlugin template) {
	 ...
	 * event.getPlayer())); if (event.getFrom().getBlockX() >> 1 ==
	 * event.getTo().getBlockX() >> 1 && event.getFrom().getBlockZ() >> 1 ==
	 * event.getTo().getBlockZ() >> 1 && event.getFrom().getWorld() ==
	 * event.getTo().getWorld()) return;
	 * template.getListenerAdapters().forEach(adapter ->
	 ...
	 * event.getPlayer())); if (!Config.useItemFallEvent) return; Item item =
	 * event.getItemDrop(); AtomicBoolean hasSendEvent = new
	 * AtomicBoolean(false); scheduleFix(100, (task, isActive) -> { if
	 * (!isActive) return; template.getListenerAdapters().forEach(adapter ->
